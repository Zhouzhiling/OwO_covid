...
class PreprocessForNN(object):
    ...
    def generate_training_data(self, mode='outbreak'):
        ...
        for i in range(len(feature)):
            ...
            elif self.mid <= np.mean(label[i]):
                mid2_feature.append(feature[i])
                mid2_label.append(label[i])
        ...
        if mode == 'outbreak':
            ...
            scalered_feature = np.array(mid2_feature)
            scalered_label = np.array(mid2_label)
            scalered_feature = self.scaler_feature.fit_transform(scalered_feature)
            scalered_label = self.scaler_label.fit_transform(scalered_label)
            return scalered_feature, scalered_label
        elif mode == 'mid':
    ...
    def generate_testing_data(self, mode='outbreak'):
        ...
        for i in range(len(feature)):
            ...
            elif self.mid <= np.mean(feature[i][7:14]):
                mid2_feature.append(feature[i])
                mid2_FIPS.append(FIPS_list[i])
        ...
        if mode == 'outbreak':
            ...
            mid2_base = [np.mean(mid2_feature[i][7:14]) for i in range(len(mid2_feature))]
            scalered_feature = np.array(mid2_feature)
            scalered_feature = self.scaler_feature.fit_transform(scalered_feature)
            return scalered_feature, pd.Series(mid2_FIPS), mid2_base
        elif mode == 'mid':
